...
# These are didactic ball-park values, good enough to reproduce sensible speed
# distributions and pressures for dilute noble gases.
GASES = {
    "argon":   {"mass_amu": 39.948, "radius_nm": 0.171},
    "helium":  {"mass_amu": 4.0026, "radius_nm": 0.140},
    "neon":    {"mass_amu": 20.180, "radius_nm": 0.154},
    "krypton": {"mass_amu": 83.798, "radius_nm": 0.202},
    "xenon":   {"mass_amu": 131.29, "radius_nm": 0.216},
}
...
def gas_properties(name: str) -> dict:
    """Return ``{"mass_kg": ..., "radius_m": ...}`` for a named reference gas."""
    key = name.lower()
    if key not in GASES:
        raise ValueError(
    ...
    g = GASES[key]
    return {"mass_kg": g["mass_amu"] * AMU, "radius_m": g["radius_nm"] * NM}
